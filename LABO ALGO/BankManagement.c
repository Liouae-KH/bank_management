  ...
  if (check_age(dob)==1){
    ...
    while(1){
      ...
      if(gard_id==-1){
        ...
        // check guardian exists and has account
        if (check_id(gard_id)==1 && guardian_has_account(gard_id)){
          acnt.type='M';
          break;
        } else {
          printf(" <erorr> guardian ID not found or guardian has no bank account.\n <enter> ");
        }
      }
    }
  }
...
void edit_bank_account() {
  ...
  FILE *acnt_file_edit = fopen("accounts.dat","rb");
  ...
  if (acnt_file_temp == NULL) { fclose(acnt_file_edit); return; }
  ...
  while(fread(&acnt_edit, sizeof(struct account), 1, acnt_file_edit) == 1){
    if (accounr_id == acnt_edit.ID){
      ...
      while(1){
        ...
        acnt_edit.type = toupper(acnt_edit.type);
        if(acnt_edit.type=='P' || acnt_edit.type=='C'){ break; }
        else { printf(" <erorr> invalid input. \n <enter> "); }
      }
    }
  ...
  if(!found){
    printf("<erorr> account not found.\n");
...
void transfere_funds(int sender_ID) {
    ...
    else { printf("reciver account not found\n"); return; }
    ...
    if(!transfere_funds_file){ printf("error opening file\n"); return; }
    struct account sender, receiver;
    int sender_found=0, receiver_found=0;
    while(fread(&sender, sizeof(struct account),1,transfere_funds_file)==1){
    ...
    while(fread(&receiver, sizeof(struct account),1,transfere_funds_file)==1){
        if(receiver.ID == reciver_ID){
            receiver_found=1;
            break;
        }
    }
    ...
    if(sender.blocked==1 || receiver.blocked==1){
        printf("one account is blocked\n");
        fclose(transfere_funds_file);
        return;
    }
    ...
    // rewrite all accounts with updated balances
    rewind(transfere_funds_file);
    FILE* temp_trens_file = fopen ("accounts_temp.dat","wb");
...
void block_account(int enter_id) {
  ...
  FILE *acnt_file_block = fopen("accounts.dat","rb");
  FILE *acnt_file_block_temp = fopen("accounts_temp.dat","wb");
  if(!acnt_file_block || !acnt_file_block_temp){ return; }
...
void delete_bank_account(int enter_id) {
  printf(" <menu/account_management/delete_account> are you sure tou want to delete the account? enter y for yes or n for no.");
  char check;
  scanf(" %c",&check);
  if (check != 'y'){return;}

  FILE *acnt_file_delete = fopen("accounts.dat","rb");
  FILE *acnt_file_delete_temp = fopen("accounts_temp.dat","wb");
  if(!acnt_file_delete || !acnt_file_delete_temp){ return; }
...
void bank_account_manegment() {
  ...
  while(1){
    ...
    switch (mode) {
      ...
      case 7:{
        int acc;
        printf("enter account ID to block: ");
        scanf("%d",&acc);
        clearScreen();
        block_account(acc);
        break;
